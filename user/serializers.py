...
class ReturnMemberSerializer(serializers.ModelSerializer):
    ...
    def get_queryset(self):
        # Filter to include only users with role_status='ASSIGNED'
        queryset = super().get_queryset().filter(role_status=User.ROLE_STATUS.ASSIGNED)
        return queryset
...
class RoleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if attrs["name"].title() in roles:  # convert to sentence case before checking
            raise serializers.ValidationError(
                "Role name is already taken, please try another."
            )
    ...
    def create(self, validated_data):
        ...
        return Role.objects.create(
            name=validated_data["name"].title(), permissions=permissions
        )
...
class InvitationSerializer(ResendOtpSerializer):
    ...
    def validate(self, obj):
        ...
        if obj["role"] not in available_roles:
            raise serializers.ValidationError(
                f"{obj['role']} is not a valid role, please check again."
            )

        if "alternative_role" in obj:
            if (
                obj["alternative_role"]
                and obj["alternative_role"] not in available_roles
            ):
                raise serializers.ValidationError(
                    f"{obj['alternative_role']} is not a valid role, please check again."
                )
    ...
    def create(self, validated_data):
        ...
        role = get_roles(
            name=validated_data["role"].title()
        )  # fetch the role queryset using the name
...
class ResendInvitationSerializer(serializers.Serializer):
    ...
    def validate(self, obj):
        ...
        if user.first().invitation_status == User.INVITATION_STATUS.USED:
            raise serializers.ValidationError(
                "This user has accepted the invitation, please use the forgot password option."
            )
    ...
    def update(self, instance, validated_data):
        with transaction.atomic():
            ...
            instance.is_superuser = (
                True if super_admin.alternative_role is not None else False
            )
            instance.role_status = User.ROLE_STATUS.ASSIGNED
            ...
            if super_admin.alternative_role is not None:
                ...
                super_admin.role_status = User.ROLE_STATUS.ASSIGNED
